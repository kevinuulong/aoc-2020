...
// 		var isParent = true;
// 		bagRules.forEach((rules) => {
// 			if (rules.indexOf(type) != -1) {
// 				isParent = false;
// 			}
// 		})
// 		if (isParent) parents.push(type);
// 	})
// 	return parents;
// }

function searchRules(types, type, term) {
	if (!types[type]) return false;
	if (types[type].includes(term)) return true;
	for (const child of types[type]) {
		if (searchRules(types, child, term)) return true;
	}
	return false;
...
function part1(term) {
	var count = 0;
	for (const type in rulesObj) {
		count += searchRules(rulesObj, type, term);
	}
	return count;
}
...
// function searchRules(bagType, rulesList) {
// 	if (rulesList.length == 1) {
// 		return (rules.indexOf(bagType) != -1);
// 	}
// 	rulesList.forEach(rules => {
// 		if (rules.indexOf(bagType) != -1) return true;
// 		else if (rules.length > 0) {
// 			searchRules(bagType, rules);
// 		}
// 	})
// 	return false;
// }
...
// 		if (searchRules(bagType, bagRules)) count++;
// 	});
// }
...
// console.log(count);
// function part1(bagType) {
// 	if (findParents(bagType).length == 0) {
// 		count++;
// 	} else {
// 		findParents(bagType).forEach((newBagType) => {
// 			part1(newBagType);
// 		});
// 	}
// 	return count;
